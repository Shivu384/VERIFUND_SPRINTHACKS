                ...
                if task_result.get("status", "").lower() == "completed":
                    ...
                    if result.get("account_exists", "").upper() == "YES":
                        if norm_bank_name == norm_user_name:
                            profile.acc_num = 'Yes'   # Add this field to your model
                            profile.save()
                            request.session["bank_verified"] = True
                            return redirect('predict_threshold')  # ACTUALLY REDIRECT HERE
                        else:
                            context['error'] = (
...
def borrower_dashboard(request):
    try:
        user_profile = request.user.userprofile  # Access via userprofile (not profile)
    except UserProfile.DoesNotExist:
        messages.error(request, "Please complete your profile first.")
        return redirect('update_profile')

    # Check if the user's bank account is verified
    if user_profile.acc_num != 'Yes':
    ...
    try:
        lending_data = request.user.lendingmldata
        if not lending_data.model_score:
            messages.error(request, "Please predict your credit threshold first.")
            return redirect('predict_threshold')
